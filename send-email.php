  ...
  function sendEmail($ebook, $email)
  {
    ...
    $content .= "<div style='margin-top:3rem;'><a href='https://teliossagi.com/download.php?file=" . $ebook . ".pdf' style='padding: 12px 32px; border-radius: 50px; color: #ffffff; background-color: #e43c5c; text-decoration: none;'>Click to download eBook</a></div>";
    $content .= "</div>";
    $content .= "<div><p style='text-align: center;'>234 (0) 809 453 9393 <br> Lagos, Nigeria</p></div>";
    $content .= "</td></tr></table>";


    $header = "From:no-reply@teliossagi.com \r\n";
    ...
    if ($retval == true) {
      ...
      header("Location: /?customer=" . $email . "&reference=" . $_GET['reference']);
    } else {
